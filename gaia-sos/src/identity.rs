...
pub struct Constraints {
    pub purpose: String,
    pub no_actuator: bool,
}
impl Constraints {
    pub fn advise_only(purpose: impl Into<String>) -> Self {
        Self {
    ...
    fn narrows(&self, parent: &Self) -> bool {
        self.purpose == parent.purpose && (!self.no_actuator || parent.no_actuator)
    }
}
...
impl Capability {
    ...
    pub fn attenuates(&self, parent: &Capability) -> bool {
        self.issuer == parent.subject
            && self.parent.as_ref() == Some(&parent.id)
            && self.resource == parent.resource
            && self.operations.is_subset(&parent.operations)
            && self.constraints.narrows(&parent.constraints)
            && self.not_before >= parent.not_before
            && self.expires_at <= parent.expires_at
            && self.delegation.remaining_depth < parent.delegation.remaining_depth
    }
}
...
impl CapabilityAuthority {
    ...
    pub fn issue(&mut self, capability: Capability) -> Result<(), AuthorizationError> {
        if let Some(parent_id) = &capability.parent {
            let parent = self
                .capabilities
                .get(parent_id)
                .ok_or(AuthorizationError::UnknownCapability)?;
            if self.is_revoked(parent_id) || !capability.attenuates(parent) {
                return Err(AuthorizationError::InvalidDelegation);
            }
        }
    ...
    pub fn authorize(
        ...
        if capability.allows(subject, resource, operation, now) {
            Ok(())
        } else {
    ...
    pub fn revoke(&mut self, capability_id: &CapabilityId) -> Result<u64, AuthorizationError> {
        if !self.capabilities.contains_key(capability_id) {
            return Err(AuthorizationError::UnknownCapability);
        }
    ...
    pub fn is_revoked(&self, capability_id: &CapabilityId) -> bool {
        ...
        while let Some(id) = cursor {
            ...
            cursor = self
                .capabilities
                .get(&id)
                .and_then(|capability| capability.parent.clone());
        }
        false
    }
}
