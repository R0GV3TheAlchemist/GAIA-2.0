...
fn unpublished_invisible_and_minors_not_ranked() {
    let cards = [
        SkillCard { published: false, age_years: 34, skill: "x".into() },
        SkillCard { published: true, age_years: 16, skill: "y".into() },
    ];
    assert_eq!(SkillCard::search(&cards).len(), 1);
    assert_eq!(cards[1].rank_in_marketplace().unwrap_err(), SkillError::ChildRank);
