use gaia_earth::{OutcomeKind, ScenarioEngine, ScenarioLibrary, SimError, SimMode, SourceKind};
...
fn what_if_net_zero_returns_ensemble_with_uncertainty() {
    ...
    assert!(run
        .outcomes
        .iter()
        .any(|d| d.kind == OutcomeKind::TippingRisk));
    assert!(run.outcomes.iter().all(|d| d.uncertainty > 0.0));
    assert!(run
        .outcomes
        .iter()
        .all(|d| d.source == SourceKind::Synthetic));
}
